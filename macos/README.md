...
- 原生截图覆盖层：框选、带半透明候选蒙层的悬停窗口选择、窗口置顶后重新截图、内联标注、OCR、复制和直接保存。
- 最近转录保留最近 20 条，支持双击左键复制。
- 智能整理支持自动范围规则，可按目标窗口或本次口述内容选择整理模式。
...
当前本地发布版本：`1.9.3 (540)`。构建脚本默认会覆盖安装到 `/Applications/TypeWhale Pro.app`；如只想生成 `macos/TypeWhale Pro.app`，设置 `TYPESPEAKER_SKIP_INSTALL=1`。
